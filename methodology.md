...
9. **The index returns to neutral before the impacts do, and nothing
   in this brief says so.** Every number we publish describes the state
   of the ocean now, or a forecast of it. Several of the impacts our
   other channels track lag that state by many months, and the largest
   lag is long enough to invert the reading entirely.

   The clearest case is Amazon fire. Fire in the southern Amazon is
   governed by how much water the PRECEDING wet season put into the
   ground, not by concurrent sea surface temperature, so the fire
   response to an El Nino arrives roughly 15 months after onset, in the
   following dry season (Chen et al. 2017, Nature Climate Change; the
   pan-tropical cascade puts Northern Hemisphere South America at +90%
   emissions at 9 months and Southern Hemisphere South America at +86%
   at 15 months). Both 1997-98 and 2023-24 behaved this way: the
   catastrophe landed in the year after the peak.

   For the 2026-27 event that means the principal southern-Amazon
   window is roughly July to October 2027, and our own CPC strength
   table already carries MAM 2027 at 0% probability of a very strong
   event. **A reader, or a downstream channel, taking the headline
   ladder as a live risk gauge in mid-2027 will read near-zero at the
   moment the impact peaks.** The index will not be wrong; it will be
   answering a different question from the one being asked of it.

   We do not currently fix this, and adding a lagged impact index is
   not in scope for V1.5: it would be original modelling of exactly the
   kind the build philosophy rules out on a sample of four events. What
   we can do is refuse the inference. Any channel building on our
   numbers should read this limitation first, and no reader-facing copy
   should present a falling ladder as falling risk without saying which
   risk.

   Verified against our own data on 2026-08-30, and the verification
   also weakened the mechanism above: the proximate variable, southern
   Amazon wet-season soil moisture (ERA5 layer 3, Dec to May), ranked
   18th driest of 35 years for 2026, and 2025 had the 3rd driest
   recharge on record alongside the lowest Amazon fire year on record.
   The lag structure in the published literature is well evidenced.
   Our attempt to confirm its proximate mechanism from soil moisture
   alone is not, so the timing claim above should be cited to Chen and
   not to us.
10. **2015 is not a clean development-year analog, and the chart does not
    say so.** The analog chart aligns each event by calendar month within
    its development year, March as origin. That basis is right: ENSO is
    phase-locked to the annual cycle, so March 1997 and March 2026 are
    genuinely comparable ocean states, and aligning by "months since
    onset" instead would be worse.

    The problem is which years the basis is applied to. Three of the four
    plotted events enter their development year from a cool or neutral
    ocean: 1997 at DJF -0.41, 2023 at -0.55, 2026 at -0.39. **2015 enters
    at +0.73, already above the El Nino threshold**, because the 2014-15
    event never fully decayed (its prior NDJ was +0.80). The first-quarter
    rise makes the difference plain: 1997 +0.71, 2023 +0.74, 2026 +0.85,
    and 2015 only +0.13.

    **So 2015 is a second-year continuation rather than a fresh
    development, and it sits on the chart labelled "2015 develop year"
    beside two events that are.** The consequence is specific: any claim
    that 2026 "overtook 2015" is partly an artefact of 2015 having
    started 1.1 C higher and risen slowly, not a statement about 2026's
    vigour. The 1997 and 2023 comparisons carry no such problem and
    should carry any argument being made.
...
12. **An analog average is only usable if the analogs are exchangeable in
    the variable that drives the answer, and ours were not.** Recorded
    because it produced materially wrong probabilities that reached
    Kristjan before being caught, and he caught them by intuition rather
    than by any check of ours.

    We estimated the peak as ASO plus the ASO-to-peak gain, taking the
    gain as the mean over past El Nino events (+0.41, sd 0.14, n=4
    strong). That put our peak at +3.12 and gave P(peak > 4.0) of about 1
    percent against the models' 14 to 43 percent. **The disagreement was
    ours, not theirs.**

    **The gain is not a constant. It scales with the subsurface heat
    available to drive it:**

        gain = 0.263 * OHC_at_ASO + 0.277     r = +0.47, n = 12

    The same relationship holds, more strongly, for the JJA-to-ASO step
    (r = +0.59, p = 0.045). **2026's heat content at +2.19 is roughly
    double any analog's**, and the model predicts a gain of +0.85 where we
    had used +0.47. Corrected, the peak distribution moves to about +3.55
    and sits between CPC's anchor and the model consensus at every
    threshold instead of below both.

    **The failure was not the arithmetic. We had already established that
    2026 holds the highest subsurface heat in the 47-year record, in this
    same document, and then averaged over events with half of it.** The
    data was in hand and the conditioning was not done.

    **Two limits on the correction, which must travel with it.** The
    relationship is r = +0.47 at p = 0.125, suggestive and not
    significant. And 2026's OHC sits a full unit beyond the fitted range,
    so applying it is extrapolation of exactly the kind limitation 11
    warns against. The gain could be +0.85 or it could saturate.

    **The general rule: before averaging over analogs, ask which variable
