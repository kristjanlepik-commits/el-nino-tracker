# El Niño Probability Tracker, week of 2026-07-27
...
Headline numbers below are CPC-derived after translating from RONI bins to traditional ONI thresholds, then fitting a skew-normal distribution to the nine bin probabilities and evaluating its survival function at each threshold. RONI-to-traditional-ONI offset is +0.80°C, the live tropical-mean SST anomaly observed for the week of 2026-07-22 (CPC). ECMWF SEAS5 member counts in caveat 2 are a second quantitative cross-check.

- **At least moderate (>+1.0°C peak)**: 100%
- **Strong (>+1.5°C peak)**: 100%
- **Very strong / super (>+2.0°C peak)**: 98% (CPC anchor 99%, 6-model consensus 98%, deflection -1.3 ppt)
- **1997/2015 magnitude (>+2.5°C peak)**: 92% (CPC anchor 89%, 6-model consensus 92%, deflection +3.0 ppt)
- **Beyond instrumental record (>+3.0°C peak)**: 80% (CPC anchor 62%, 6-model consensus 83%, deflection +18.1 ppt)
- **Far beyond record (>+3.5°C peak)**: 55% (CPC anchor 27%, 6-model consensus 60%, deflection +27.9 ppt)
...
4. The +3.0°C and +3.5°C buckets are the most model-dependent numbers in the headline. +3.0°C exceeds every event in the instrumental record (1997 ~2.4, 2015 ~2.6, 1877 ~2.5 on HadISST), and it sits beyond CPC's published strength bins (which top out at >=2.0 RONI), so its CPC anchor (62%) is a deep skew-normal tail extrapolation. Under the consensus weighting the +3.0 bucket (80%) is driven mostly by direct model member counts, not that extrapolation. The +3.5°C bucket (55%, added 2026-07-06 once the July SEAS5 run pushed the top of the distribution past where +3.0 discriminates) is even more model-driven: its CPC anchor (27%) is effectively zero (that threshold is ~+3.0 RONI, far past CPC's top bin), so it is almost entirely direct model member counts above +3.5. Read it as 'where the hot models cluster,' not a calibrated probability, and note it leans hardest on the July ECMWF run.

## 2. Physical state panel

| Indicator | Current (week of ~22 Apr 2026) | 1997 same week | 2015 same week |
|---|---|---|---|
| Niño 3.4 weekly (traditional) | +2.2°C | -0.1°C | +0.6°C |
...
| Cumulative westerly wind anomaly since Mar 1 | 438 m/s·days (CWWA, ERA5 130E-150W, vs 1991-2020 climo) | 622 | 567 |
...
**CWWA note:** Live ERA5 daily 850 hPa zonal wind through 2026-07-21, area-meaned over 5N-5S, 130E-150W and integrated for positive (westerly) anomalies vs the 1991-2020 same-calendar-day climatology. At the same calendar date, 2026 CWWA (438) tracks closest to 2015 (567); other reference years: 1997 (622), 2023 (111), 2025 (10). Caveat: CWWA is a cumulative-area-mean metric and systematically understates transient localized westerly wind bursts, including those occurring just outside the 5N-5S band. A short intense burst can generate a downwelling Kelvin wave that does substantial physical work even when it barely moves the cumulative integral. For the operational read on whether ENSO development is on track, the surfacing-Kelvin-wave evidence in heat content (above) is at least as informative as this metric. See the WWB row below for the spatial-peak event count and analyst read (v1.7, complementary to CWWA).

**WWB events (spatial-peak detection, v1.7):** 10 westerly wind burst events detected since Mar 1, 2026. Detection: sliding 5x10 deg sub-region area-mean anomaly over 10N-10S, 130E-150W; dual threshold (5 m/s sustained for at least 5 days, peak day above 7 m/s) with peak-detection plus a 10-day recovery interval between events. Analogs (events to same calendar date): 1997 (11), 2015 (12), 2023 (10), 2025 (5).
  ...
  - 2026-07-13 to 2026-07-21, 9 days, peak 17.36 m/s, peak day 2026-07-17
...
On count: 2026 has 10 events so far; analogs at the same calendar date: 1997 (11), 2015 (12), 2023 (10), 2025 (5). v1.7's peak-detection algorithm with a 10-day recovery interval splits sustained westerly periods into distinct bursts where v1.6 collapsed them. The count is now reasonably comparable across years, but peak amplitude remains the cleaner single number.
...
**RONI->trad ONI offset changed: +0.7°C -> +0.8°C.** This shifts headline buckets even without underlying probability changes; flag in editorial.

**New agency releases since last issue:**

- **iri**: prior issued 2026-06-22, now issued 2026-07-20.

**Unchanged since last issue:** cpc_strength, ecmwf, bom.

**Physical state deltas:**

- nino34_weekly_traditional: 2.1 -> 2.2 (delta +0.1)
- cwwa_ms_days: 396.54 -> 437.97 (delta +41.43)
- wwb_events_since_mar1: 9 -> 10 (delta +1)
...
- **oisst_weekly**: fetched live, issued 2026-07-22.
...
- **era5_wwe**: fetched live, issued 2026-07-21.
- **era5_burst**: fetched live, issued 2026-07-22.
- **oni_history**: fetched live, issued 2026-06-30.
- **nmme**: fetched live, issued 2026-07-08.

---

*Generated by run_brief.py from sources.py + probs.py + analog.py. Methodology version 1.9. RONI offset +0.80°C (live, week of 2026-07-22). Next issue: Mon 4 May 2026 (per Monday cadence; first batch run is off-schedule).*
