# El Niño Probability Tracker, week of 2026-08-10
...
Headline numbers below are CPC-derived after translating from RONI bins to traditional ONI thresholds, then fitting a skew-normal distribution to the nine bin probabilities and evaluating its survival function at each threshold. RONI-to-traditional-ONI offset is +0.80°C, the live tropical-mean SST anomaly observed for the week of 2026-08-05 (CPC). ECMWF SEAS5 member counts in caveat 2 are a second quantitative cross-check.

- **At least moderate (>+1.0°C peak)**: 100%
- **Strong (>+1.5°C peak)**: 100%
- **Very strong / super (>+2.0°C peak)**: 100% (CPC anchor 99%, 6-model consensus 100%, deflection +0.8 ppt)
- **1997/2015 magnitude (>+2.5°C peak)**: 98% (CPC anchor 89%, 6-model consensus 100%, deflection +9.3 ppt)
- **Beyond instrumental record (>+3.0°C peak)**: 94% (CPC anchor 62%, 6-model consensus 100%, deflection +32.3 ppt)
- **Far beyond record (>+3.5°C peak)**: 70% (CPC anchor 27%, 6-model consensus 78%, deflection +43.4 ppt)
...
4. The +3.0°C and +3.5°C buckets are the most model-dependent numbers in the headline. +3.0°C exceeds every event in the instrumental record (1997 ~2.4, 2015 ~2.6, 1877 ~2.5 on HadISST), and it sits beyond CPC's published strength bins (which top out at >=2.0 RONI), so its CPC anchor (62%) is a deep skew-normal tail extrapolation. Under the consensus weighting the +3.0 bucket (94%) is driven mostly by direct model member counts, not that extrapolation. The +3.5°C bucket (70%, added 2026-07-06 once the July SEAS5 run pushed the top of the distribution past where +3.0 discriminates) is even more model-driven: its CPC anchor (27%) is effectively zero (that threshold is ~+3.0 RONI, far past CPC's top bin), so it is almost entirely direct model member counts above +3.5. Read it as 'where the hot models cluster,' not a calibrated probability, and note it leans hardest on the July ECMWF run.

## 2. Physical state panel

| Indicator | Current (week of 2026-08-05) | 1997 same week | 2015 same week |
|---|---|---|---|
| Niño 3.4 weekly (traditional) | +2.6°C | +1.6°C | +1.5°C |
...
| Cumulative westerly wind anomaly since Mar 1 | 519 m/s·days (CWWA, ERA5 130E-150W, vs 1991-2020 climo) | 663 | 639 |
...
**CWWA note:** Live ERA5 daily 850 hPa zonal wind through 2026-08-04, area-meaned over 5N-5S, 130E-150W and integrated for positive (westerly) anomalies vs the 1991-2020 same-calendar-day climatology. At the same calendar date, 2026 CWWA (519) tracks closest to 2015 (639); other reference years: 1997 (663), 2023 (132), 2025 (11). Caveat: CWWA is a cumulative-area-mean metric and systematically understates transient localized westerly wind bursts, including those occurring just outside the 5N-5S band. A short intense burst can generate a downwelling Kelvin wave that does substantial physical work even when it barely moves the cumulative integral. For the operational read on whether ENSO development is on track, the surfacing-Kelvin-wave evidence in heat content (above) is at least as informative as this metric. See the WWB row below for the spatial-peak event count and analyst read (v1.7, complementary to CWWA).

**WWB events (spatial-peak detection, v1.7):** 11 westerly wind burst events detected since Mar 1, 2026. Detection: sliding 5x10 deg sub-region area-mean anomaly over 10N-10S, 130E-150W; dual threshold (5 m/s sustained for at least 5 days, peak day above 7 m/s) with peak-detection plus a 10-day recovery interval between events. Analogs (events to same calendar date): 1997 (12), 2015 (13), 2023 (11), 2025 (5).
  ...
  - 2026-07-25 to 2026-08-04, 11 days, peak 20.85 m/s, peak day 2026-07-31
...
On count: 2026 has 11 events so far; analogs at the same calendar date: 1997 (12), 2015 (13), 2023 (11), 2025 (5). v1.7's peak-detection algorithm with a 10-day recovery interval splits sustained westerly periods into distinct bursts where v1.6 collapsed them. The count is now reasonably comparable across years, but peak amplitude remains the cleaner single number.
...
|---|---|---|---|---|---|---|---|---|
| NCEP CFSv2 | 32 | 4.24°C | 100% | 100% | 100% | 100% | 100% | 100% |
| CanESM5 | 20 | 3.41°C | 100% | 100% | 100% | 100% | 100% | 40% |
| GEM5.2-NEMO | 20 | 4.07°C | 100% | 100% | 100% | 100% | 100% | 100% |
| NCAR CCSM4 | 10 | 3.32°C | 100% | 100% | 100% | 100% | 100% | 30% |
| NCAR CESM1 | 10 | 4.74°C | 100% | 100% | 100% | 100% | 100% | 100% |
| **Consensus (equal model wt)** | 5 models | **3.96°C** | 100% | 100% | 100% | 100% | 100% | 74% |

**Consensus read:** The multi-model consensus puts 100% of members above +2.5°C (1997/2015 magnitude), and 100% above +3.0°C, which would exceed every event in the instrumental record (1997 ~2.4, 2015 ~2.6, 1877 ~2.5 on HadISST). These are directly-counted member fractions, not tail extrapolations. As of methodology v1.8 the NMME suite feeds the section-1 headline directly: the multi-model consensus deflection blends these models with ECMWF SEAS5 at weight 0.85. This panel shows the per-model breakdown behind that consensus, including the spread between the hot models (CFSv2, NCAR) and the cooler outliers (CanESM5).
...
- **ecmwf**: prior issued 2026-07-01, now issued 2026-08-01.

**Unchanged since last issue:** cpc_strength, iri, bom.

**Physical state deltas:**

- nino34_weekly_traditional: 2.3 -> 2.6 (delta +0.3)
- nino34_weekly_roni: 1.5 -> 1.8 (delta +0.3)
- heat_content_0_300m_estimate: 2.94 -> 2.96 (delta +0.02)
...
- **oisst_weekly**: fetched live, issued 2026-08-05.
...
- **era5_wwe**: fetched live, issued 2026-08-04.
- **era5_burst**: fetched live, issued 2026-08-05.
- **oni_history**: fetched live, issued 2026-07-31.
- **nmme**: fetched live, issued 2026-08-08.

---

*Generated by run_brief.py from sources.py + probs.py + analog.py. Methodology version 1.9. RONI offset +0.80°C (live, week of 2026-08-05). Next issue: Mon 4 May 2026 (per Monday cadence; first batch run is off-schedule).*
