# El Niño Probability Tracker, week of 2026-09-07
...
- **Beyond instrumental record (>+3.0°C peak)**: 96% (CPC anchor 86%, 6-model consensus 98%, deflection +10.5 ppt)
- **Far beyond record (>+3.5°C peak)**: 75% (CPC anchor 53%, 6-model consensus 79%, deflection +22.3 ppt)
- **Nothing to compare it to (>+4.0°C peak)**: 31% (CPC anchor 19%, 5-model consensus 33%, deflection +11.6 ppt)
...
4. The +3.0°C and +3.5°C buckets are the most model-dependent numbers in the headline. +3.0°C exceeds every event in the instrumental record (1997 +2.37, 2015 +2.59), and it sits beyond CPC's published strength bins (which top out at >=2.0 RONI), so its CPC anchor (86%) is a deep skew-normal tail extrapolation. Under the consensus weighting the +3.0 bucket (96%) is driven mostly by direct model member counts, not that extrapolation. The +3.5°C bucket (75%, added 2026-07-06 once the July SEAS5 run pushed the top of the distribution past where +3.0 discriminates) is even more model-driven: its CPC anchor (53%) is effectively zero (that threshold is ~+3.0 RONI, far past CPC's top bin), so it is almost entirely direct model member counts above +3.5. Read it as 'where the hot models cluster,' not a calibrated probability, and note it leans hardest on the July ECMWF run.

## 2. Physical state panel

| Indicator | Current (week of 2026-09-02) | 1997 same week | 2015 same week |
|---|---|---|---|
| Niño 3.4 weekly (traditional) | +2.7°C | +2.1°C | +1.7°C |
| Niño 3.4 weekly (RONI) | +1.8°C | n/a (pre-RONI) | n/a (pre-RONI) |
| 0-300m heat content anomaly | +3.22°C (CPC monthly, 180W-100W, vs 1981-2010 climo) | +1.79°C | +1.97°C |
| Cumulative westerly wind anomaly since Mar 1 | (CWWA fetch failed; not computed this run) | n/a | n/a |

**Heat content note:** At +3.22°C, 2026 exceeds both 1997 (+1.79°C) and 2015 (+1.97°C) at the same calendar month, running ahead of either super-event analog at this stage of development.

**CWWA note:** Not computed this run: the ERA5 cumulative westerly wind anomaly could not be fetched and no usable cached series was available (FetcherTimeout: fetcher exceeded 1500s budget). The WWB row below is the independent wind-forcing indicator and is unaffected when it renders.
...
|---|---|---|---|---|---|---|---|---|---|
| NCEP CFSv2 | 32 | 3.75°C | 100% | 100% | 100% | 100% | 100% | 97% | 3% |
...
| **Consensus (equal model wt)** | 5 models | **3.92°C** | 100% | 100% | 100% | 100% | 98% | 77% | 33% |
...
- **ecmwf**: prior issued 2026-08-01, now issued 2026-09-01.
- **nmme**: prior issued 2026-08-08, now issued 2026-09-08.
    - consensus above +3.0: 94% -> 98% (Delta +4)
    - consensus above +3.5: 62% -> 77% (Delta +15)
    - consensus above +4.0: 43% -> 33% (Delta -10)
...
- heat_content_0_300m_estimate: 3.2 -> 3.22 (delta +0.02)
...
- **oisst_weekly**: fetched live, issued 2026-09-02.
...
- **era5_wwe**: NO USABLE DATA. Live fetch failed and no cache was readable, so this source fell back to sources.py seed values. Error: FetcherTimeout: fetcher exceeded 1500s budget.
- **era5_burst**: NO USABLE DATA. Live fetch failed and no cache was readable, so this source fell back to sources.py seed values. Error: FetcherTimeout: fetcher exceeded 1800s budget.
- **oni_history**: fetched live, issued 2026-08-31.
- **nmme**: fetched live, issued 2026-09-08.

---

*Generated by run_brief.py from sources.py + probs.py + analog.py. Methodology version 1.11. RONI offset +0.90°C (live, week of 2026-09-02). Next issue: Mon 4 May 2026 (per Monday cadence; first batch run is off-schedule).*
