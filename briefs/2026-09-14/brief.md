# El Niño Probability Tracker, week of 2026-09-14
...
- NOAA CPC strength table, NDJ 2026-27 (RONI): super 93%, strong 7%, moderate 0%, weak El Niño 0%, neutral 0%, La Niña 0%. Issued 2026-09-10.
- IRI plume, DJF 2026-27: El Niño 100%, neutral 0%, La Niña 0%. Issued 2026-08-19. Strength not broken out in the public Quick Look.
- BoM ENSO Outlook, issued 2026-09-01: El Niño continues to strengthen while IOD values ease back into the neutral range. Categorical only.
- ECMWF SEAS5, run 2026-09-01: 51-member SEAS5 ensemble for 2027-03: median Niño 3.4 anomaly +3.71 deg C; 51/51 members above +1.5 (~100% above +2.0, ~100% above +2.5).

**Caveats this issue:**

1. The +2.5°C bucket carries a 96-99% range. It comes from a bootstrap that perturbs CPC's published bin probabilities by Gaussian noise (sigma 1 percentage point, matching CPC's whole-percent reporting precision) and refits the skew-normal each time. The range therefore reflects reporting-quantization uncertainty in CPC's table, not underlying forecast uncertainty.
2. ECMWF SEAS5 vs CPC, upper tail above +2.5°C trad ONI: SEAS5 has 51/51 members (100%) at 2027-03 (max available lead). CPC's NDJ 2026-27 bucket lands at 96-99%. We subtract SEAS5's own model climatology, which removes its known ENSO warm bias; an observational-climatology subtraction would put SEAS5 higher still. Real disagreement to surface, not a number to average.
...
4. The +3.0°C and +3.5°C buckets are the most model-dependent numbers in the headline. +3.0°C exceeds every event in the instrumental record (1997 +2.37, 2015 +2.59), and it sits beyond CPC's published strength bins (which top out at >=2.0 RONI), so its CPC anchor (85%) is a deep skew-normal tail extrapolation. Under the consensus weighting the +3.0 bucket (96%) is driven mostly by direct model member counts, not that extrapolation. The +3.5°C bucket (72%, added 2026-07-06 once the July SEAS5 run pushed the top of the distribution past where +3.0 discriminates) is even more model-driven: its CPC anchor (15%) is effectively zero (that threshold is ~+3.0 RONI, far past CPC's top bin), so it is almost entirely direct model member counts above +3.5. Read it as 'where the hot models cluster,' not a calibrated probability, and note it leans hardest on the July ECMWF run.

## 2. Physical state panel

| Indicator | Current (week of 2026-09-09) | 1997 same week | 2015 same week |
|---|---|---|---|
| Niño 3.4 weekly (traditional) | +2.9°C | +1.8°C | +1.7°C |
| Niño 3.4 weekly (RONI) | +2.0°C | n/a (pre-RONI) | n/a (pre-RONI) |
| 0-300m heat content anomaly | +3.23°C (CPC monthly, 180W-100W, vs 1981-2010 climo) | +1.79°C | +1.97°C |
| Cumulative westerly wind anomaly since Mar 1 | (CWWA fetch failed; not computed this run) | n/a | n/a |

**Heat content note:** At +3.23°C, 2026 exceeds both 1997 (+1.79°C) and 2015 (+1.97°C) at the same calendar month, running ahead of either super-event analog at this stage of development.

**CWWA note:** Not computed this run: the ERA5 cumulative westerly wind anomaly could not be fetched and no usable cached series was available (FetcherTimeout: fetcher exceeded 1500s budget | no cache at /home/runner/work/el-nino-tracker/el-nino-tracker/.fetch_cache/era5_wwe_last_good.json). The WWB row below is the independent wind-forcing indicator and is unaffected when it renders.

## 2b. Multi-model consensus (NMME)

North American Multi-Model Ensemble, init 2026090800 (issued 2026-09-08). Peak Nino 3.4 over Nov 2026 - Feb 2027 (NDJ-DJF), region 5N-5S, 170W-120W. Each cell is the percent of that model's ensemble members whose peak exceeds the threshold. Anomalies are vs each model's own hindcast climatology (same convention as SEAS5).

| Model | Members | Peak ONI (mean) | >+1.0 | >+1.5 | >+2.0 | >+2.5 | >+3.0 | >+3.5 | >+4.0 |
|---|---|---|---|---|---|---|---|---|---|
| NCEP CFSv2 | 32 | 3.76°C | 100% | 100% | 100% | 100% | 100% | 100% | 3% |
| CanESM5 | 20 | 3.71°C | 100% | 100% | 100% | 100% | 100% | 90% | 0% |
| GEM5.2-NEMO | 20 | 4.12°C | 100% | 100% | 100% | 100% | 100% | 100% | 80% |
| NCAR CCSM4 | 10 | 3.18°C | 100% | 100% | 100% | 100% | 90% | 0% | 0% |
| NCAR CESM1 | 10 | 4.15°C | 100% | 100% | 100% | 100% | 100% | 100% | 80% |
| **Consensus (equal model wt)** | 5 models | **3.92°C** | 100% | 100% | 100% | 100% | 98% | 78% | 33% |

**Consensus read:** The multi-model consensus puts 100% of members above +2.5°C (1997/2015 magnitude), and 98% above +3.0°C, which would exceed every event in the instrumental record (1997 +2.37, 2015 +2.59). These are directly-counted member fractions, not tail extrapolations. As of methodology v1.8 the NMME suite feeds the section-1 headline directly: the multi-model consensus deflection blends these models with ECMWF SEAS5 at weight 0.85. This panel shows the per-model breakdown behind that consensus, including the spread between the hot models (CFSv2, NCAR) and the cooler outliers (CanESM5).

**Panel caveats:**

- NMME updates monthly (around the 8th). This init (2026090800) predates the late-May model runs discussed publicly; the next init will capture those.
...
**Methodology version bumped: 1.11 -> 1.12.** Check the methodology change log for whether headline-bucket comparability to last issue is affected; math changes break comparability, chart or prose changes do not.

**New agency releases since last issue:**

- **cpc_strength**: prior issued 2026-08-13, now issued 2026-09-10.
    - NDJ 1.0to1.5: 1% -> 0% (Delta -1)
    - NDJ 1.5to2.0: 9% -> 7% (Delta -2)
    - NDJ >=2.0: 90% -> 93% (Delta +3)

**Unchanged since last issue:** iri, ecmwf, bom, nmme.

**Physical state deltas:**

- nino34_weekly_traditional: 2.7 -> 2.9 (delta +0.2)
- nino34_weekly_roni: 1.8 -> 2.0 (delta +0.2)
- heat_content_0_300m_estimate: 3.22 -> 3.23 (delta +0.01)
...
- **cpc_strength**: fetched live, issued 2026-09-10.
- **oisst_weekly**: fetched live, issued 2026-09-09.
- **heat_content**: fetched live, issued 2026-08-31.
- **iri**: fetched live, issued 2026-08-19.
- **bom**: fetched live, issued 2026-09-01.
- **ecmwf_seas5**: fetched live, issued 2026-09-01.
- **era5_wwe**: NO USABLE DATA. Live fetch failed and no cache was readable, so this source fell back to sources.py seed values. Error: FetcherTimeout: fetcher exceeded 1500s budget | no cache at /home/runner/work/el-nino-tracker/el-nino-tracker/.fetch_cache/era5_wwe_last_good.json.
- **era5_burst**: NO USABLE DATA. Live fetch failed and no cache was readable, so this source fell back to sources.py seed values. Error: FetcherTimeout: fetcher exceeded 1800s budget | no cache at /home/runner/work/el-nino-tracker/el-nino-tracker/.fetch_cache/era5_burst_last_good.json.
- **oni_history**: fetched live, issued 2026-08-31.
- **nmme**: fetched live, issued 2026-09-08.

---

*Generated by run_brief.py from sources.py + probs.py + analog.py. Methodology version 1.12. RONI offset +0.90°C (live, week of 2026-09-09). Next issue: Mon 4 May 2026 (per Monday cadence; first batch run is off-schedule).*
