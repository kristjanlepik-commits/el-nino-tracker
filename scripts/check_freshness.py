...
LAYERS = [
    ...
    {"path": "docs/pacific-sst.json", "as_of": _from_key("observation_date"),
     # 8, NOT 14. Design's point and it is arithmetic rather than taste:
     # this sits on a WEEKLY page, so any budget above 7 days guarantees
     # the picture can be older than the issue around it and still score
     # green. It was green at 13 days showing a fortnight-old ocean.
     # Eight allows one late run without allowing a stale issue.
     #
     # No longer "refreshed by hand": the weekly job now regenerates it
     # immediately before the render, so a breach here means that step
     # failed rather than that somebody forgot.
     "max_age": 8, "owner": "DESIGN",
     "what": "front page Pacific SST field, regenerated by the Monday job"},
]
