...
.gk{padding-bottom:0}
/* THE UNIT SQUARES BUILT A TOWER. 196px of label and 54px of figure plus
   two 20px gaps leave 48px of a 338px phone column for the squares, so a
   19px square wrapped at TWO per row and this summer's 34 days ran
   seventeen rows down the screen, past the fold, beside an empty left
   half. The comparison is the point of the block and it cannot be seen
   when the two quantities do not share a screen.
   Label and figure take the top line, the squares wrap across the full
   width beneath them: fourteen per row, so 34 is three rows against one
   for a typical summer, and the ratio reads at a glance again. */
.urow{grid-template-columns:minmax(0,1fr) auto;
 grid-template-areas:"key val" "sq sq";gap:9px 14px;align-items:baseline}
.uk{grid-area:key;text-align:left}
.un{grid-area:val}
.ug{grid-area:sq}
...
for name, v in sorted(C.items()):
    ...
    # THE METHOD SENTENCE IS HEAT'S FIELD, NOT MY PROSE. It used to read
    # "Every year is counted to 29 August, so a part-finished summer is
    # never set against complete ones", hardcoded, on every page. For the
    # seven southern cities that described a truncation THAT NEVER RAN:
    # effective_cut returns None for a season running October to January,
    # so the cut clips nothing and all 123 window days of every season are
    # counted. The like-with-like guarantee held, for a different reason.
    #
    # I refused to write the corrected sentence when I found this, and
    # that was the right call rather than a cautious one: my guess would
    # have been "counted to 31 January", which is also false. Nothing is
    # counted TO anything for these cities. Heat answered from the code
    # and emitted it, so the claim is now re-derivable by anyone.
    #
    # Printed verbatim. Rewording a method claim in the renderer is how a
    # page ends up asserting something no code does.
    _cb = v.get("counting_basis") or {}
    _basis = (_cb.get("basis") or "").strip()
    if not _basis:
        raise SystemExit(
            f"{name}: counting_basis.basis is missing. It is on heat's "
            f"TOP_LEVEL_CONTRACT, so this is a payload fault rather than "
            f"an optional field, and the page must not fall back to a "
            f"sentence about how days are counted that nothing verifies.")
    _basis = _basis[0].upper() + _basis[1:]

    # THE DEFINITIONS ROWS CARRY THE SAME CLAIM IN FEWER WORDS, and that
    # is where this defect survived its own fix once already. Heat's
    # August miss, in CLAUDE.md at their request: the sentence was
    # corrected, the definitions row said the same thing differently, and
    # the page they had sent back shipped with the claim intact.
    #
    # Two components printing one quantity is the cheapest detector we
    # have, so both branch on the one field rather than one being fixed
    # and the other being the thing nobody was looking at.
    _clips = bool(_cb.get("cut_clips_the_window"))
    _wlab = _cb.get("window_label") or ""
    _range_txt = (f"to {v['counted_to']}" if _clips
                  else f"{_wlab}, complete")
    _counted_txt = (f"counted to {v['counted_to']}" if _clips
                    else f"whole {_wlab} window, {_cb.get('window_days')} days")
    ...
    method = (f'<p class="cap">Hot here means {th}&nbsp;&deg;C or above, a level '
              ...
              f'the number of days clearing it has. {_basis}. '
...
<span style="text-align:right">{_range_txt}</span>
...
<span style="text-align:right">{_counted_txt}</span>
</div>
