...
Branch `worktree-tls-rebrand`, commit `82e7c5b`. Generated by
`design/make_review_bundle.py`; do not hand-edit.

This exists because GitHub's directory and compare pages are blocked on
the reviewing side while raw file URLs are not. Everything reviewable is
concatenated here so it can be fetched with one request.

Contents: the README, the complete token set, the complete public
stylesheet, and the five component builders worth arguing about.

The four rendered pages are too large to inline. Fetch individually:

    .../worktree-tls-rebrand/design/review/archive.html        (8 KB)
    .../worktree-tls-rebrand/design/review/weekly-issue.html   (52 KB)
    .../worktree-tls-rebrand/design/review/methodology.html    (53 KB)
    .../worktree-tls-rebrand/design/review/front-page.html     (125 KB)

all under `https://raw.githubusercontent.com/kristjanlepik-commits/el-nino-tracker/`
...
`/*VARS_LIGHT*/` and `/*VARS_DARK*/` markers are where `css_variables()`
output lands.
